...
ngOnInit(){this.loadRecipes();this.refreshSales();this.loadSellers();this.startAutoRefresh();} ngOnDestroy(){this.destroy$.next();this.destroy$.complete();}
...
openTableSelector(){this.tableSelectorOpen.set(true); if(this.availableTables().length>0)return; this.loadTables();}
...
private refreshSales(silent=false){this.loadingSales.set(true);this.salesService.getSales().subscribe({next:s=>{this.ngZone.run(()=>{const normalized=(s??[]).map(sale=>({ ...sale, details:[...(sale.details??[])] })); this.sales.set(normalized);this.loadingSales.set(false);this.cdr.markForCheck();});},error:e=>{this.ngZone.run(()=>{this.loadingSales.set(false);if(!silent)this.pushToast('error',this.readLoadSalesError(e));this.cdr.markForCheck();});}})}
private loadRecipes(silent=false){
	...
	this.recipeService.getRecipes().subscribe({
		...
		error:()=>{
			this.ngZone.run(()=>{
				...
				if(!silent)this.pushToast('error','No se pudieron cargar las recetas.');
				this.cdr.markForCheck();
			});
		}
	})
}
private loadSellers(silent=false){
	...
	this.sellerService.getSellers().subscribe({
		...
		error:(error)=>{console.error('Error cargando vendedores', error);this.ngZone.run(()=>{this.loadingSellers.set(false);if(!silent)this.pushToast('error','No se pudieron cargar los vendedores.');this.cdr.markForCheck();});}
	})
}
...
private startAutoRefresh(){
	interval(3000).pipe(takeUntil(this.destroy$)).subscribe(()=>{
		...
		if(this.availableTables().length>0||this.tableSelectorOpen()) this.loadTables(true);
	});
}
private loadTables(silent=false){this.loadingTables.set(true); this.tableService.getTables().subscribe({next:t=>{this.ngZone.run(()=>{this.availableTables.set([...(t??[])]);this.loadingTables.set(false);this.cdr.markForCheck();});},error:()=>{this.ngZone.run(()=>{this.loadingTables.set(false);if(!silent)this.pushToast('error','No se pudieron cargar las mesas.');this.cdr.markForCheck();});}})}
