...
import { Component, OnDestroy, OnInit, computed, inject, signal, ChangeDetectorRef } from '@angular/core';
...
openTableSelector(){this.tableSelectorOpen.set(true); if(this.availableTables().length>0)return; this.loadingTables.set(true); this.tableService.getTables().subscribe({next:t=>{this.availableTables.set(t);this.loadingTables.set(false);},error:()=>{this.loadingTables.set(false);this.pushToast('error','No se pudieron cargar las mesas.');}})}
closeTableSelector(){this.tableSelectorOpen.set(false);} selectTable(t:RestaurantTable){if(!this.isTableAvailable(t))return this.pushToast('warning','Debes seleccionar una mesa disponible.'); this.selectedTableId.set(t.id); this.selectedTableLabel.set(this.tableLabel(t)); this.pushToast('success','Mesa seleccionada correctamente.'); this.closeTableSelector();}
...
selectSeller(s:Seller){if(!this.isSellerActive(s))return; this.selectedSellerId.set(s.id); this.selectedSellerName.set(this.sellerDisplayName(s)); this.pushToast('success','Vendedor seleccionado correctamente.'); this.closeSellerSelector();}
...
refreshPreparations(saleId:string){this.loadingTraceSaleId.set(saleId); this.salesService.getSalePreparations(saleId).subscribe({next:t=>{this.loadingTraceSaleId.set(''); this.preparationTracesBySaleId.update(s=>({...s,[saleId]:t})); if(t.some(x=>x.status==='REJECTED'))this.pushToast('warning','Una o más recetas fueron rechazadas por inventario.');},error:()=>{this.loadingTraceSaleId.set(''); this.pushToast('error','No se pudo cargar la trazabilidad.');}});}
...
private refreshSales(){this.loadingSales.set(true);this.salesService.getSales().subscribe({next:s=>{this.sales.set(s??[]);this.loadingSales.set(false);},error:e=>{this.loadingSales.set(false);this.pushToast('error',this.readLoadSalesError(e));}})}
private loadRecipes(){
	this.loadingRecipes.set(true);
	this.recipeError.set(false);
	this.recipeService.getRecipes().subscribe({
		next:(r:RecipeResponse[])=>{
			this.recipes.set(r.map(x=>({id:x.id,name:x.name,price:x.sellingPrice,active:x.active,visibleInMenu:x.visibleInMenu})));
			this.recipeNameById.set(r.reduce((a,c)=>({...a,[c.id]:c.name}),{}));
			this.loadingRecipes.set(false);
			try{ this.cdr.detectChanges(); }catch{}
			// Force re-emit objects and a browser repaint to avoid cases where text
			// doesn't render until user interaction (some browsers need a reflow).
			try{
				this.recipes.update(list=>list.map(i=>({ ...i })));
				// small repaint hack (use setTimeout to avoid compiler warnings)
				if (typeof window !== 'undefined') try { setTimeout(()=>{ void document.body.offsetHeight; }, 0); } catch {}
			}catch{}
		},
		error:()=>{
			this.loadingRecipes.set(false);
...
private loadSellers(){
	...
	this.sellerService.getSellers().subscribe({
		next:s=>{
			...
			try{ this.cdr.detectChanges(); }catch{}
			try{ this.availableSellers.update(list=>list.map(i=>({ ...i }))); if(typeof window !== 'undefined') try { setTimeout(()=>{ void document.body.offsetHeight; }, 0); } catch {} }catch{}
		},
		error:(error)=>{console.error('Error cargando vendedores', error);this.loadingSellers.set(false);this.pushToast('error','No se pudieron cargar los vendedores.');}
	})
}
