...
export class SalesPosPage implements OnInit, OnDestroy {
  ...
  readonly selectedTableId = signal('Mesa 1');
  ...
  private loadRecipes(): void {
    ...
    this.recipeService.getRecipes().subscribe({
      next: (recipes: RecipeResponse[]) => {
        const mapped = recipes.map(r => ({ id: r.id, name: r.name, price: r.sellingPrice, active: r.active, visibleInMenu: r.visibleInMenu }));
        this.recipes.set(mapped);
        this.recipeNameById.set(recipes.reduce((acc, curr) => ({ ...acc, [curr.id]: curr.name }), {}));
  ...
  private buildPayload(): SaleRequest | null {
    if (!this.selectedSellerId() || !this.selectedLocationId() || !this.selectedTableId()) return this.pushToast('error', 'Vendedor, sede y mesa o referencia son obligatorios.'), null;
  ...
  private readError(error: unknown): string { const err = error as HttpErrorResponse; return err.error?.message ?? 'Ocurrió un error inesperado.'; }
}
